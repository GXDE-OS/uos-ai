...
void SkillInfo::collectFiles(const QDir &dir) const
{
    ...
    for (const QFileInfo &fileInfo : fileInfos) {
        ...
        if (fileName.startsWith("LICENSE", Qt::CaseInsensitive) || fileName.startsWith("README", Qt::CaseInsensitive) || fileName.compare("SKILL.md", Qt::CaseInsensitive) == 0) {
            continue;
        }
        /* 添加完整绝对路径 */
...
{
    QJsonArray result;
    const auto skills = sortSkills(m_skills.values());
    ...
    for (const auto &skill : skills) {
        ...
        obj["path"] = skill.path;
        obj["source"] = skill.source;
        obj["enabled"] = skill.enabled;
        result.append(obj);
    }
...
QJsonArray SkillsManager::enabledSkillsData() const
{
    QJsonArray result;
    QList<SkillInfo> enabledSkillsList;

    for (const auto &skill : m_skills) {
        if (skill.enabled) {
            enabledSkillsList.append(skill);
        }
    }

    const auto sorted = sortSkills(enabledSkillsList);
...
QJsonObject SkillsManager::addSkillForWeb()
{
    ...
    DFileDialog dlg;
    dlg.setDirectory(QStandardPaths::standardLocations(QStandardPaths::HomeLocation).first());
    ...
    dlg.setObjectName("fileDialogAddSkill");
    ...
    QString errorMsg;
    QString skillName;
    bool success = addSkill(selected.first(), &errorMsg, &skillName);
...
bool SkillsManager::moveToTrash(const QString &path)
{
    ...
    if (!file) {
        qCWarning(skillsManager) << "Failed to create GFile for:" << path;
        return false;
    }
    ...
    if (!success) {
        if (error) {
            qCWarning(skillsManager) << "Failed to move to trash using GIO:" << error->message;
            g_error_free(error);
        } else {
            qCWarning(skillsManager) << "Failed to move to trash using GIO:" << path;
        }
        return false;
    }
