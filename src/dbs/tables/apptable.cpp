...
bool AppTable::save()
{
    ...
    if (DaoClient::getInstance().execBatchSync(
                "INSERT OR REPLACE INTO app (uuid, name, desc, llmid, cmd, ext) VALUES (:uuid, :name, :desc, :llmid, :cmd, :ext) ;", {}
    , {{"uuid", uuid()}, {"name", name()}, {"desc", desc()}, {"llmid", llmid()}, {"cmd", cmd()}}, result, msg, "basic")) {
        return true;
    } else {
        qInfo() << "sql error: " << msg;
    }
...
bool AppTable::update()
{
    ...
    QString msg;
    if (DaoClient::getInstance().execBatchSync(
    "UPDATE app SET name=:name, desc=:desc, llmid=:llmid, cmd=:cmd, ext=:ext WHERE uuid=:uuid ;", {{"uuid", uuid()}}
    , {{"name", name()}, {"desc", desc()}, {"llmid", llmid()}, {"cmd", cmd()}, {"cmd", ext()}}, result, msg, "basic")) {
        return true;
    } else {
        qInfo() << "sql error: " << msg;
    }
...
AppTable  AppTable::create(const QString &uuid, const QString &name, const QString &desc, const QString &llmid, const QString &cmd, const QString &ext)
{
    AppObject obj;
    obj.uuid = uuid;
    obj.name = name;
    ...
    "SELECT * FROM app WHERE uuid=:uuid;", {{"uuid", uuid}}, {}, result, msg, "basic")) {
        if (result && result->size() > 0) {
            ...
            auto obj = AppTable::create(
                           ...
                           , item.value("llmid").toString(), item.value("cmd").toString(), item.value("ext").toString());
            return obj;
        }
    } else {
        qInfo() << "sql error: " << msg;
