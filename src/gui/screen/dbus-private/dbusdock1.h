...
Q_DECLARE_METATYPE(MyDockRect)
inline QDBusArgument &operator<<(QDBusArgument &argument, const MyDockRect &rect)
{
...
inline const QDBusArgument &operator>>(const QDBusArgument &argument, MyDockRect &rect)
{
    argument.beginStructure();
    argument >> rect.x >> rect.y >> rect.width >> rect.height;
    argument.endStructure();
...
inline QDebug operator<<(QDebug deg, const MyDockRect &rect)
{
...
public:
    ...
    Q_PROPERTY(MyDockRect FrontendWindowRect READ frontendWindowRect NOTIFY FrontendWindowRectChanged)
    inline MyDockRect frontendWindowRect() const
    { return qvariant_cast< MyDockRect >(property("FrontendWindowRect")); }
