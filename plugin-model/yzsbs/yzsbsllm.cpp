...
YzsbsLLM::YzsbsLLM(QObject *parent)
    : QObject(parent)
    , LLMModel()
{
...
QString YzsbsLLM::model() const
{
    return modelID();
}

QJsonObject YzsbsLLM::generate(const QString &content, const QVariantHash &params, LLMModel::streamFuncion stream, void *user)
{
    QJsonObject sendObj;
    ...
    QUrl url("http://10.10.16.11:8502/onestop-deploy/chat");
    ...
    QObject::connect(this, &YzsbsLLM::sigAbort, reply, &QNetworkReply::abort);
    QObject::connect(this, &YzsbsLLM::sigAbort, &loop, &QEventLoop::quit);
    ...
    reference.insert("type", 4);
    {
        ...
        if (!refVar.isEmpty()) {
            QJsonObject item;
            item.insert("docPath", "一站式交付方案知识库");
