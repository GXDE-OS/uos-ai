...
// Change the logging category to follow uosai prefix
    ...
    , d(new DLayerShellWindowPrivate(window))
{
    ...
    if (waylandWindow) {
        qCDebug(layershellwindow) << "Wayland window detected, setting up layer shell integration";
        connect(waylandWindow, &QtWaylandClient::QWaylandWindow::destroyed, this, [&](){
                s_map.remove(window);
                deleteLater();
        });
        ...
        if (!shellIntegration) {
            ...
            if (!shellIntegration->initialize(waylandWindow->display())) {
                qCCritical(layershellwindow) << "Failed to initialize layer shell integration";
                delete shellIntegration;
                shellIntegration = nullptr;
                qCWarning(layershellwindow) << "failed to init dlayershell intergration";
                return;
            }
            qCInfo(layershellwindow) << "Layer shell integration initialized successfully";
        }
        waylandWindow->setShellIntegration(shellIntegration);
    }
#ifdef BUILD_WITH_X11
    else if (auto xcbWindow = dynamic_cast<QNativeInterface::Private::QXcbWindow*>(window->handle())) {
        qCInfo(layershellwindow) << "X11 window detected, setting up layer shell emulation";
...
DLayerShellWindow* DLayerShellWindow::get(QWindow* window)
{
    ...
    qCDebug(layershellwindow) << "Creating new DLayerShellWindow for window";
